...
customElements.define(
	...
	class TextImage extends HTMLElement {
		constructor() {
			...
			if (elImage.length === 0) elImage = this.querySelectorAll('img')
			if (elImage.length > 1)
			...
			if (this.designVariant !== 'before' && this.designVariant !== 'after')
				throw new TypeError(
					'The +imgPosition value is either "before" or "after".'
				)
		...
		`
			...
			if (this.designVariant === 'before') position = 'afterbegin'
			this.querySelector('section').insertAdjacentElement(
				position,
				this.elImage
			)
			// place the textual content in its box
			this.querySelector('.axa-text-image__text').insertAdjacentElement(
				'afterbegin',
				this.elContent
			)
			this.querySelector('.axa-text-image__text').insertAdjacentElement(
